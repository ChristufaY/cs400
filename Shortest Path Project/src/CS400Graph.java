...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            {
              this.dataSequence.add(data); //makes a data sequence with the same sequence as the path that as being copied
            }
            this.dataSequence.add(extendBy.target.data);  //adds the extension target to the data sequence
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
      ...
      Path dijkstrapath = new Path(startvertex);
      Hashtable<T, Vertex> visited = new Hashtable<T, Vertex>();
      pq.add(dijkstrapath);
      Hashtable<Vertex, Path> pathstorer = new Hashtable<Vertex, Path>(); //stores the shortest path to a vertex with the vertex as key
      while (!pq.isEmpty())
      {
        ...
        {
          Edge e = current.end.edgesLeaving.get(i);
          if (!visited.contains(e.target))
          {
            Path newcurrent = new Path(current, e);
            if (pathstorer.get(e.target) != null) //if we have found a path to this vertex already
            {
              if (newcurrent.distance < pathstorer.get(e.target).distance)  //if new path is shorter than current shortest path
              {
                pathstorer.remove(e.target);
                pathstorer.put(e.target, newcurrent);
                pq.add(newcurrent);
              }
            }
            else
      ...
      if (pathstorer.get(vertices.get(end)) != null)
      {
        return pathstorer.get(vertices.get(end)); //find the shortest path using pathstorer hashtable
      }
      throw new NoSuchElementException();
    }
    ...
    public List<List<T>> getAllPaths(T start, T end)
    {
      ...
      Path thepath = new Path(vertices.get(start));
      getAllPathsHelper(start, end, thepath);
      List<List<T>> solution = pathList;
      pathList = new ArrayList<List<T>>();
      return solution;
    }
    private void getAllPathsHelper(T start, T end, Path p)
    {
      Hashtable<T, Vertex> visited = new Hashtable<T, Vertex>();
      if (p.end.data.equals(end))
      {
        pathList.add(p.dataSequence);
        return;
      }
      else
      {
        for (T data: p.dataSequence)
        {
          visited.put(data, vertices.get(data));
        }
        for (int i = 0; i < p.end.edgesLeaving.size(); i++)
        {
          if (!visited.contains(p.end.edgesLeaving.get(i).target))
          {
            Path newpath = new Path(p, p.end.edgesLeaving.get(i));
            getAllPathsHelper(start, end, newpath);
          }
        }
      }
    }
    public List<T> getAllVertexData()
    {
