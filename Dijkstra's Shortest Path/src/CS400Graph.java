...
public class CS400Graph<T> implements GraphADT<T> {
	...
	protected class Path implements Comparable<Path> {
		...
		public Path(Path copyPath, Edge extendBy) {
			...
			//this.start = new Vertex(copyPath.start.data);
//			for(int a = 0; a < copyPath.start.edgesLeaving.size(); a++) {
//				this.start.edgesLeaving.add(copyPath.start.edgesLeaving.get(a));
//			}
			//System.out.println("DataSeq: " + this.dataSequence);
			this.dataSequence = new LinkedList<>();
...
//			this.end = new Vertex(extendBy.target.data);
//			for(int j = 0; j < extendBy.target.edgesLeaving.size(); j++) {
//				this.end.edgesLeaving.add(extendBy.target.edgesLeaving.get(j));
//			}
			//System.out.println("DataSeqAfter: " + this.dataSequence);
		...
		public int compareTo(Path other) {
			//System.out.println("CT other: " + other.end);
	...
	protected Path dijkstrasShortestPath(T start, T end) {    
		// checks if start and end exist.
		//System.out.println("SP start: " + start);
		//System.out.println("SP end: " + end);
		...
		Path temp = new Path(this.vertices.get(start));
		//System.out.println("Temp: " + temp.end.data);
		Hashtable<T, Vertex> visited = new Hashtable<T, Vertex>();
		frontier.add(temp);
		while(!(frontier.isEmpty())) {
			temp = frontier.poll();
			//System.out.println("StartAfterPoll: " + temp.start);
			//System.out.println("DistanceAfterPoll: " + temp.distance);
			//System.out.println("SeqAfterPoll: " + temp.dataSequence);
			//System.out.println("TempEndAfterPoll: " + temp.end);
			visited.put(temp.end.data, new Vertex(temp.end.data));
			if(temp.end.data == end) {
				return temp;
			}
			for(int i = 0; i < temp.end.edgesLeaving.size(); i++) {
				if(!(visited.containsKey(temp.end.edgesLeaving.get(i).target.data)))
					frontier.add(new Path(temp, temp.end.edgesLeaving.get(i)));
			}
		}
